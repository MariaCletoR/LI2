...
void menu_inicial() {
    ...
    while(1) {
        clear();
        mvprintw(yMax / 2 - 1, xMax / 2 - 4, "ROGUELIKE"); // Centraliza o título
        mvprintw(yMax / 2 + 1, xMax / 2 - 2, "1. Jogar"); // Centraliza as opções
        mvprintw(yMax / 2 + 2, xMax / 2 - 6, "2. Personagens");
        mvprintw(yMax / 2 + 3, xMax / 2 - 2, "3. Sair");
        refresh();
        ...
        switch(opcao) {
            case '1': // Jogar
                ...
                return;
            case '2': // Personagens
                // Aqui você pode adicionar o código para mostrar as informações dos personagens.
                getch();
                break;
            case '3': // Sair
                endwin();
